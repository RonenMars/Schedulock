...
final class SnapshotTests: XCTestCase {
    ...
    func testRenderedImageHasNonUniformPixels() {
        for type in TemplateType.allCases {
            let image = renderTemplate(type, events: sampleEvents)

            XCTAssertNotNil(image, "Template \(type.displayName) should produce an image")

            guard let image = image,
                  let cgImage = image.cgImage else {
                XCTFail("Failed to get CGImage for \(type.displayName)")
                continue
            }
            ...
            guard let context = CGContext(
                ...
                XCTFail("Failed to create context for pixel reading")
                continue
            }
            ...
            for pos in samplePositions {
                let offset = (pos.y * width + pos.x) * bytesPerPixel
            ...
            // Verify not all colors are identical (image has content variation)
            let uniqueColors = Set(colors.map { "\($0.r),\($0.g),\($0.b)" })
            XCTAssertGreaterThan(
                uniqueColors.count,
                1,
                "Template \(type.displayName) should have non-uniform pixels (got \(uniqueColors.count) unique colors from \(colors.count) samples)"
            )
        }
    }
